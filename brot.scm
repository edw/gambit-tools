...
(define (brot cx cy scale theta)
  (loop ((let: cx-pxs (fl/ (fl- (fixnum->flonum brot-width) 1.0) 2.0))
         (let: cy-pxs (fl/ (fl- (fixnum->flonum brot-height) 1.0) 2.0))
         (let: sin-theta (flsin theta))
         (let: cos-theta (flcos theta))
         ...
         (let: i1 (fl- (fl* cos-theta i0) (fl* sin-theta j0)))
         (let: j1 (fl+ (fl* cos-theta j0) (fl* sin-theta i0)))
         (let: i (fl+ cy i1))
         (let: j (fl+ cx j1)))
